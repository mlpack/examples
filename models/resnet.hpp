/**
  ...
  * @author Kartik Dutt
  * 
  * Definition of ResNet generally used for object detection.
  * 
  ...
  */
#ifndef MODELS_RESNET_HPP
#define MODELS_RESNET_HPP
...
/**
 * Definition of a ResNets.
 */
class ResNet
{
 public:
  //! Create the ResNet object.
  ResNet();
   ...
   * @param useNorm Boolean to determine use of batch Normalization.
   * @param weights One of 'none', 'imagenet'(pre-training on mnist) or path to weights.
   * @param ResNetVer Version of ResNet.
   */
  ResNet(const size_t inputChannel,
         ...
         const int ResNetVer = 1);

  //! Get Layers of the model.
  Sequential<> *GetModel() { return resNet; };
...
private:
  ...
  void ConvolutionBlock(const size_t inSize,
                        ...
                        const size_t padH = 0)
  {
    resNet->Add<Convolution<>>(inSize, outSize, kernelWidth,
        kernelHeight, strideWidth, strideHeight, padW, padH, inputWidth,
        inputHeight);
    resNet->Add<LeakyReLU<>>();
  ...
  void PoolingBlock(const size_t kernelWidth,
                    ...
                    const size_t strideHeight = 1)
  {
    resNet->Add<MaxPooling<>>(kernelWidth, kernelHeight,
...
#include "resnet_impl.hpp" // Include implementation.
